import requests
from globalVariables import base_url, generateEvaluationForm,companyUsername,password
from loginTest import get_header
url = base_url +generateEvaluationForm

def evaluationForm(data,expected_status_code):
...
def unauthorizedAccess():
    ...
    for key in expected_response_body:
        assert key in json_data, f"Expected key '{key}' in the response body"
#get evaluationForm
evaluationForm({"trainingId":1},200)    
#get evaluationForm for non existing training
evaluationForm({"trainingId":10000},400) 
#missing id
evaluationForm({},500)
#unauthorized access
unauthorizedAccess()